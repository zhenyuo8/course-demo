/**
 * @param 面向对象的单利模式模块化开发
 * @param 惰性思想的应用
 * @param 同模块之间方法函数的调用this.fn
 *
 */
var utils = (function () {//单例模式
    var flg = 'getComputedStyle' in window;//惰性思想
    function makeArray(arg) {//arguments函数中的传参形式：形参和arguments
        var ary = [];
        if (flg) {
    ...
    function jsonParse(jsonStr) {//判断当前浏览器是否有JSON内置属性，如果有直接使用内置属性中的JSON.parse方法，如果没有，则用字符串拼接的方法将JSON格式的字符串转换为JSON格式的字符串对象
    ...
    function offset(curEle) {
        ...
        var par = curEle.offsetParent;//获取当前元素的上一个定位父级
        while (par) {//判断上一个定位父级是否存在，并判断浏览器是否是IE 8.0，由于IE8.0版本中它的offsetLeft中包含了clientLeft同理offsetTop；
            ...
            l += par.offsetLeft;
            t += par.offsetTop;
            par = par.offsetParent;//由于做了累加操作，每次累加结束后都要重新获取下一个定位父级，否则进入死循环
        }
        return {left: l, top: t};
    }
    ...
    function rnd(n, m) {
        ...
        n = Number(n);
        m = Number(m);
        //1.判断传的参数是不是有效数字，如果有一个不是邮箱数字，则返回一个0-1之间的随机小数，代表传参错误；
        if (isNaN(n) || isNaN(m)) {
        ...
        return Math.round(Math.random() * (m - n) + n);//四舍五入Math.round
    }
    ...
    function getByClass(strClass, curEle) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
            ...
            for (var j = 0; j < aryClass.length; j++) {//'box1','box2'....
                var reg = new RegExp('\\b' + aryClass[j] + '\\b');//\是转译字符 \b代表空格；匹配以空格开头，空格结尾
    ...
    function addClass(curEle, strClass) {
        ...
        for (var i = 0; i < aryClass.length; i++) {
            if (!this.hasClass(curEle, aryClass[i])) {
                curEle.className += ' ' + aryClass[i];//空字符串中必须加空格，否则字符串拼接后会出现只有一个class名，出错；
            }
        }
    }
    ...
    function setGroupCss(curEle, opt) {
        if (opt.toString() !== '[object Object]') return;//如果opt不是对象，直接阻断程序执行；
        for (var attr in opt) {
            this.setCss(curEle, attr, opt[attr])
        }
    }

    function css(curEle) {//getCss、setCss、setGroupCss三合一，判断参数属于什么类型，且参数的个数
    ...
    function prependChild(parent, newEle) {
        ...
        if (first) {//如果有第一个子元素，则将新元素插入到第一个子元素的前面
    ...
    return {
        ...
        nextAll: nextAll,
        //sibling:获取当前元素的相邻兄弟元素 prev+next
        sibling: sibling,
        //siblings:获取当前元素的所有兄弟元素 prevAll+nextAll;
        siblings: siblings,
        //index:求当前元素的索引
        index: index,
