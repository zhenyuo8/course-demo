...
(function(){
    var zhenyu = {
        ...
        Bounce: {
            easeIn: function (t, b, c, d) {
                return c - zhenyu.Bounce.easeOut(d - t, 0, c, d) + b;
            },
            ...
            easeInOut: function (t, b, c, d) {
                if (t < d / 2) {
                    return zhenyu.Bounce.easeIn(t * 2, 0, c, d) * .5 + b;
                }
                return zhenyu.Bounce.easeOut(t * 2 - d, 0, c, d) * .5 + c * .5 + b;
            }
        },
        //二次方的缓动
        Quad: {
    ...
    function move(obj,target,opt){
        ...
        if(typeof defaultOpt.effect === 'number'){
            ...
            defaultOpt.effect=ary.length>=2?zhenyu[ary[0]][ary[1]]:zhenyu[ary[0]];
        }else if(typeof defaultOpt.effect === 'object'){
            defaultOpt.effect=defaultOpt.effect.length>=2?zhenyu[defaultOpt.effect[0]][defaultOpt.effect[1]]:zhenyu[defaultOpt.effect[0]];
        }
        var begin={},change={};
        ...
        obj.timer=setInterval(function(){
            time+=10;
            //停止条件的判断
            if(time>=defaultOpt.duration){//当time的值大于设置的时间时，直接将当前元素移动到目标位置，并关闭浏览器
                utils.css(obj,target);
                clearInterval(obj.timer);
                //回调函数，并且改变回调函数中的this指向
                /*defaultOpt.callback && defaultOpt.callback.call(obj);*/
                fire(obj,'moveEnd');//订阅发布中的端口暴露，在运动结束让当前元素执行某个方法
                return;
            }
